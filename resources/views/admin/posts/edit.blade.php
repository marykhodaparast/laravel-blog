...
@section('content')
    <h3 class="text-warning edit text-right">این مطلب را ویرایش کنید.</h3>
    <form method="POST" action="{{route('post.update',['id'=>$post->id]) }}" class="needs-validation" novalidate enctype="multipart/form-data">
        @csrf
        <div class="form-group text-right">
            <label for="title">عنوان: </label>
            <input type="text" name="title" class="form-control {{$errors->has('title') ? 'is-invalid' : ''}}" id="title" placeholder="title" value="{{$post->title}}">
        </div>
        <div class="form-group text-right">
            <label for="body">بدنه: </label>
            <textarea type="text" cols="5" rows="5" class="form-control {{$errors->has('body') ? 'is-invalid' : ''}}" id="body" name="body" placeholder="body">{{$post->body}}</textarea>
        </div>
        ...
        <p class="font-weight-bold text-right">طبقه بندی ها:</p>
        <div class="form-group">
            <select name="cat" id="cat" class="browser-default custom-select">
                @foreach($categories as $category)
                    <option value="{{$category->id}}" {{$category->id==$pid?'selected':''}}>{{$category->name}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
        ...
        <img src="{{$post->thumbnail}}" alt="not found" class=" edit-image">
        <div class="form-group text-right">
